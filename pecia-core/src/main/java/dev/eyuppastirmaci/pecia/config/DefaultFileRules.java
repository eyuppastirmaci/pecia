...
public final class DefaultFileRules {
    ...
    /** Filename patterns excluded from candidate discovery. */
    ...
    public static List<String> includes() {
        return Stream.concat(
                        ...
                        BASENAMES.stream().map(name -> "**/" + name))
                .toList();
    }
    ...
    public static List<String> excludes() {
        return Stream.concat(
                        EXCLUDED_DIRECTORIES.stream().map(dir -> "**/" + dir + "/**"),
                        EXCLUDED_FILES.stream().map(name -> "**/" + name))
                .toList();
    }
}
