...
final class SqliteLexicalRetriever {
    ...
    // Keeps snapshot coordination testable while the default reader still uses the real storage connection.
    ...
    /**
     * Executes an already compiled literal MATCH expression with equal field weights.
     * Raw SQLite BM25 scores sort ascending; binary paths and chunk positions break ties before LIMIT.
     *
    ...
    /** Reads candidates and their metadata from one snapshot without taking ownership of the connection. */
    List<SearchHit> search(String matchExpression, int limit) throws SQLException {
        ...
        return storage.inScope(() -> {
            ...
            Map<Long, ChunkMetadata> metadata = metadataReader.read(candidates.stream().map(Candidate::chunkId).toList());
    ...
    private List<Candidate> readCandidates(String matchExpression, int limit) throws SQLException {
        ...
        try (var statement = connection.prepareStatement("""
            ...
            try (var rows = statement.executeQuery()) {
                while (rows.next()) {
                    candidates.add(new Candidate(rows.getLong("chunk_id"), rows.getLong("file_id"),
                            rows.getInt("chunk_index"), rows.getString("source_path"), rows.getDouble("bm25_score"),
                            rows.getString("document_type"), rows.getString("content"), rows.getInt("start_line"),
                            rows.getInt("end_line"), rows.getString("content_snippet")));
                }
            }
        }
    ...
    private static SearchHit toHit(Candidate candidate, ChunkMetadata metadata) throws SQLException {
        try {
            Chunk chunk = new Chunk(SqlitePath.decode(candidate.sourcePath()), DocumentType.valueOf(candidate.documentType()),
                    candidate.chunkIndex(), candidate.content(), new LineRange(candidate.startLine(), candidate.endLine()), metadata);
            StoredChunk stored = new StoredChunk(candidate.chunkId(), candidate.fileId(), chunk);

            return new SearchHit(stored.id(), chunk.index(), chunk.sourcePath(), chunk.documentType(),
                    chunk.sourceLocation(), chunk.metadata(), new SearchScore(candidate.bm25Score(), SearchScore.Kind.SQLITE_BM25),
    ...
    record Candidate(long chunkId, long fileId, int chunkIndex, String sourcePath, double bm25Score,
                     String documentType, String content, int startLine, int endLine, String snippet) { }
}
