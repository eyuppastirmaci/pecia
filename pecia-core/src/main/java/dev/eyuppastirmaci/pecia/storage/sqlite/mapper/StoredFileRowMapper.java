...
public final class StoredFileRowMapper implements RowMapper<StoredFile> {
    ...
    public StoredFile map(ResultSet row) throws SQLException {
        try {
            return new StoredFile(row.getLong("id"), SqlitePath.decode(row.getString("source_path")),
                    DocumentType.valueOf(row.getString("document_type")), new ContentHash(row.getString("content_hash")));
