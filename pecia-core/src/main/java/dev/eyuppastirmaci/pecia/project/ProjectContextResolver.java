...
public final class ProjectContextResolver {
    ...
    /**
     * Uses the existing nearest-config/Git-root rules for a directory target.
     * Relative store paths resolve against the loaded project root; absolute paths may be outside it.
     * Existing store ancestors are resolved to their real paths so aliases cannot hide the index from discovery.
     * Storage opening remains responsible for validating database ownership and schema.
     *
     * @throws IOException if the target is not a real directory, contains a symbolic link, or paths/config cannot be read
    ...
    public ProjectContext resolve(Path target) throws IOException {
        ...
        if (!Files.readAttributes(normalized, BasicFileAttributes.class, LinkOption.NOFOLLOW_LINKS).isDirectory()) {
            throw new IOException("Target must be a directory: " + normalized);
        }

        LoadedConfig loaded = configLoader.load(normalized);
        Path database = loaded.root().resolve(Path.of(loaded.config().storePath())).normalize();
