...
/**
 * Plain-text lexical search over an existing index, obtained from
 * {@link dev.eyuppastirmaci.pecia.storage.sqlite.SqliteStorage#lexicalSearch()}.
 * Does not own or close storage. Callers must serialize access to the storage connection;
 * this API adds no concurrent-use guarantee.
 */
public abstract class LexicalSearch {
    ...
    /**
     * Searches literal query parts with AND semantics and returns immutable BM25-ranked hits.
     * Smaller scores rank first; ties use binary source path, chunk position, then chunk ID.
     * Snippets are plain content excerpts, while locations describe the complete source chunk.
     *
     * @param request validated plain query text and result limit; raw FTS operators are not interpreted
