...
/** Selects chunking strategies by document family and optional source-file extension. */
public final class DocumentChunkerFactory {
    ...
    /** Creates a factory with non-null family strategies and no extension overrides. */
    public DocumentChunkerFactory(
            DocumentChunker textChunker, DocumentChunker markdownChunker, DocumentChunker sourceCodeChunker) {
        this(textChunker, markdownChunker, sourceCodeChunker, Map.of());
    }

    /**
     * Creates a factory with family strategies and copied, case-insensitive extension overrides.
     *
     * @throws NullPointerException if a strategy, the registry, or a registry key or value is null
     * @throws IllegalArgumentException if extension keys are invalid or duplicate after normalization
     */
    public DocumentChunkerFactory(
     ...
     * @param overlapTokens maximum best-effort overlap in content tokens, confined to oversized leaf
     *     blocks for Markdown
     * @return a factory with dedicated Markdown and source-code strategies and shared general text
     *     routing for other families
     * @throws NullPointerException if tokenCounter or its identity is null
     * @throws IllegalArgumentException if the token budget or overlap is incompatible with the
     *     tokenizer
     */
    ...
    /**
     * Creates reusable built-in strategies with explicit extension overrides for documents already
     * classified as source code.
     *
     * @param tokenCounter tokenizer used by the built-in strategies
     * @param maxTokens maximum input tokens including model-added special tokens
     * @param overlapTokens maximum best-effort overlap in content tokens, confined to oversized leaf
     *     blocks for Markdown
     * @param sourceChunkersByExtension custom strategies responsible for their own token policy,
     *     keyed by case-insensitive alphanumeric extensions with an optional leading dot
     * @return a factory with copied extension overrides and a general source-code strategy for
     *     unmatched source files
     * @throws NullPointerException if the tokenizer, its identity, the registry, a key, or a strategy
     *     is null
     * @throws IllegalArgumentException if token settings are invalid or extension keys are invalid or
     *     duplicate after normalization
     */
    public static DocumentChunkerFactory create(
    ...
    /**
     * Selects a registered final-extension strategy for source code or the document family's default
     * strategy without reading the file.
     *
     * @param document classified document whose type and source filename determine the strategy
     * @return the existing extension-specific strategy or the family fallback when no source override
     *     matches
    ...
    /**
     * Canonicalizes extension keys into an immutable registry and rejects ambiguous aliases instead
        ...
        for (Map.Entry<String, DocumentChunker> entry : extensions.entrySet()) {
            ...
            if (!extension.matches("[a-z0-9]+")) {
                throw new IllegalArgumentException(
                        "Source extension must be alphanumeric with an optional leading dot: " + entry.getKey());
            }
