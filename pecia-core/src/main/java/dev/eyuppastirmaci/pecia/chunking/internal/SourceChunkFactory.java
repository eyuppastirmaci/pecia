...
public final class SourceChunkFactory {
    ...
    /**
     * Creates a non-blank source slice that extends coverage with offsets and the caller's line mapping.
     *
     ...
     * @throws IllegalArgumentException if the slice is blank, does not extend coverage, or has invalid chunk metadata
     */
    public static Chunk create(Document document, int index, int start, int end, int coveredEnd,
                               IntUnaryOperator lineAt) {
        ...
        // The exclusive end lies outside the slice, so its preceding character determines the final line.
