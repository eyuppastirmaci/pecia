...
public final class MarkdownChunker implements DocumentChunker {
    ...
    /**
     * Splits Markdown at structural boundaries with token-bounded text fallback and overlap confined to oversized leaf blocks.
     *
     * @param document extracted document whose original text and source identity are preserved
     * @return immutable source-ordered chunks with UTF-16 offsets, line ranges, and heading paths, or empty for whitespace-only text
    ...
    private void appendFallback(Document document, SourceText boundaries, MarkdownBlockGroup group,
        ...
        for (Chunk chunk : fallback.chunk(fragment)) {
            int start = group.startOffset() + Integer.parseInt(chunk.metadata().attributes().get("startOffset"));
            int end = group.startOffset() + Integer.parseInt(chunk.metadata().attributes().get("endOffset"));
    ...
    private static void append(Document document, SourceText boundaries, int start, int end,
        ...
        chunks.add(new Chunk(document.sourcePath(), document.type(), chunks.size(),
                document.content().substring(start, end), location, metadata));
    }
}
