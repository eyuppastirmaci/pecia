...
public final class FileContentLoader {
    ...
    /**
     * Creates a loader with a positive per-file byte limit.
     *
     ...
     * @throws ExtractionException if the file is unsuitable, too large, unreadable, or changes during
     *     reading
     */
    public FileContent load(Path file) throws ExtractionException {
        ...
        try {
            ...
            throw new ExtractionException(
                    FILE_CHANGED, file, "File changed or disappeared while it was being read: " + file, failure);
        }
    ...
    /**
     * Rejects links in every component so an apparently regular path cannot escape through an
     * ancestor link.
     */
    private void rejectSymbolicLinks(Path file) throws ExtractionException {
        for (Path part = file; part != null; part = part.getParent()) {
            if (Files.isSymbolicLink(part)) {
                throw new ExtractionException(NOT_REGULAR_FILE, file, "Symbolic-link paths are not supported: " + part);
            }
        }
    }

    private ExtractionException tooLarge(Path file) {
        return new ExtractionException(TOO_LARGE, file, "File exceeds the " + maxFileBytes + " byte limit: " + file);
    }
